...
class TestStack:
    ...
    def test_full(self):
        ...
        stk = Stack([1], 1)  # Stack initialized with 1 element, limit is 1.

        assert stk.full()  # ✅ Stack should be full.
        assert stk.size() == 1  # ✅ Size should be 1.
        assert stk.pop() == 1  # ✅ Stack is now empty.
        
        stk.push(1)  # ✅ Works fine, stack now has [1].

        # Expect OverflowError when trying to push beyond the limit
